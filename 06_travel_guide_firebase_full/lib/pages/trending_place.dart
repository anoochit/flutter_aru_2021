...
class _TrendingPlacePageState extends State<TrendingPlacePage> {
  Widget trendingPlaceItem(BuildContext context, int index, List<QueryDocumentSnapshot> listPlace) {
    return InkWell(
      child: Container(
  ...
  Widget trendingPlaceList(List<QueryDocumentSnapshot> doc) {
  ...
  Widget trendingPlaceTitle(int totalPlace) {
    return Padding(
      padding: const EdgeInsets.only(left: 16.0, right: 16.0, bottom: 28.0),
      child: Text(
        'Result $totalPlace places found',
                  ...
                  if (snapshot.hasData) {
                    ...
                    return Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          trendingPlaceTitle(doc.length),
                          Expanded(
                            child: trendingPlaceList(doc),
                          ),
                        ],
                      ),
                    );
